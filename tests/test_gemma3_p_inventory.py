...
import tests.oracles.gemma3_compare as gemma3_compare
...
def _fake_gemma3_p() -> Gemma3PForCausalLM:
    ...
    model.config = SimpleNamespace(num_hidden_layers=len(model.model.layers))
...
def test_gemma3_variant_preserves_upstream_class_contracts() -> None:
    ...
    assert (
        Gemma3PForCausalLM.hf_to_vllm_mapper
        is UpstreamGemma3ForCausalLM.hf_to_vllm_mapper
    )
    assert (
...
def test_gemma3_local_inventory_only_enumerates_local_pp_layers() -> None:
    subject = _fake_gemma3_p()
    ...
    assert {spec.layer_no for spec in local_specs if spec.layer_no >= 0} == {1}
...
def test_gemma3_compare_uses_head_dtype_for_logits(monkeypatch) -> None:
    ...
    monkeypatch.setattr(gemma3_compare.torch, "empty", fake_empty)
    monkeypatch.setattr(
        "vllm.distributed.get_tensor_model_parallel_world_size",
        lambda: 1,
    )

    gemma3_compare.Gemma3CompareForCausalLM.allocate_compare_buffers(
        subject,
        8,
        vllm_config,
    )
    ...
    assert any(
        shape == (4, 1024) and kwargs["dtype"] is torch.float32
        for shape, kwargs in allocations
    )


def test_gemma3_compare_handles_pipeline_stage_without_logits() -> None:
    subject = SimpleNamespace(
    ...
    assert (
        gemma3_compare.Gemma3CompareForCausalLM.compute_logits(
            subject,
            torch.zeros(1, 1),
        )
        is None
...
class _Rotary(nn.Module):
    ...
    def forward(
        ...
        self.events.extend([("rotary_q", q.clone()), ("rotary_k", k.clone())])
        return q + 100, k + 200
