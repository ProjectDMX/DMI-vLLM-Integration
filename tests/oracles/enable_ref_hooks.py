...
def enable_ref_hooks(
    ...
    if "qwen2_moe" in basename:
        hooks = list(hooks)
        # Qwen2-MoE uses RoPE and has no materialized positional-embedding
        # hook.  Its ref subclass allocates buffers from REF_CONFIG directly,
        # so it does not need source rewriting.
        hooks = [hook for hook in hooks if hook != "pos_embed"]
        for extra in ("router_logits", "topk_ids", "topk_weights"):
            if extra not in hooks:
                hooks.append(extra)
    enabled = set(hooks)
    config_driven = "qwen2_moe" in basename
    ...
    found_hooks: set[str] = set(enabled) if config_driven else set()
    new_lines: list[str] = []
    for line in lines:
        ...
        if m:
            ...
            if not config_driven and hook_name in enabled:
