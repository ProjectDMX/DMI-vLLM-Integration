"""CPU contracts for Gemma 4 decoder monitoring support."""
...
import dmi_vllm_integration.models.gemma4 as gemma4_p
import vllm.model_executor.models.gemma4 as upstream_gemma4
...
def test_gemma4_constructs_dmi_classes_before_compile_wrapper() -> None:
    causal_params = signature(Gemma4PForCausalLM.__init__).parameters
    outer_params = signature(
        Gemma4PForConditionalGeneration.__init__
    ).parameters

    assert causal_params["model_type"].default is Gemma4PModel
    assert (
        outer_params["language_model_type"].default
        is Gemma4PForCausalLM
    )


def test_gemma4_causal_constructor_uses_backbone_factory() -> None:
    observed = {}
...
def test_gemma4_outer_constructor_uses_language_model_factory(
    monkeypatch,
) -> None:
    observed = {}
    ...
    config = _outer_config(audio_config=None)
    vllm_config = SimpleNamespace(
    ...
    monkeypatch.setattr(gemma4_p, "initialize_model", initialize_model)
    monkeypatch.setattr(
        Gemma4PForConditionalGeneration,
        "_mark_tower_model",
...
def test_gemma4_compile_wrapper_captures_dmi_backbone_forward(
    monkeypatch,
) -> None:
    captured = []
    ...
    Gemma4PModel(vllm_config=vllm_config)

    assert captured == [Gemma4PModel.forward]


def test_gemma4_instrumenter_rejects_late_class_swapping() -> None:
    with pytest.raises(TypeError, match="language model must be constructed"):
        gemma4_p._instrument_gemma4_language_model(SimpleNamespace())

    language_model = Gemma4PForCausalLM.__new__(Gemma4PForCausalLM)
    ...
    with pytest.raises(TypeError, match="backbone must be constructed"):
        gemma4_p._instrument_gemma4_language_model(language_model)
...
def test_gemma4_rejects_the_unimplemented_moe_block(overrides) -> None:
    config = _outer_config(text_config=_text_config(**overrides))
    with pytest.raises(NotImplementedError, match="enable_moe_block=False"):
        _require_supported_gemma4_e2b_config(config, _parallel())


@pytest.mark.parametrize(
    "text_overrides,outer_overrides,parallel_overrides,dtype",
    [
        ({"hidden_size": 2048}, {}, {}, None),
        ({"layer_types": ["full_attention"] * 35}, {}, {}, None),
        ({"quantization_config": {"quant_method": "fp8"}}, {}, {}, None),
        ({}, {"image_token_id": 1}, {}, None),
...
def test_gemma4_does_not_reject_upstream_owned_or_shared_runtime_options(
    text_overrides,
    outer_overrides,
    parallel_overrides,
    dtype,
    ...
    config = _outer_config(
        text_config=_text_config(**text_overrides),
        **outer_overrides,
    )
    _require_supported_gemma4_e2b_config(
        config,
...
def test_gemma4_rejects_unimplemented_fast_prefill() -> None:
...
def test_gemma4_compare_buffers_omit_disabled_token_ids(monkeypatch) -> None:
    subject = Gemma4CompareForConditionalGeneration.__new__(
        Gemma4CompareForConditionalGeneration
    )
    nn.Module.__init__(subject)
    subject.language_model = _fake_language_model()
    subject.language_model.hook_token_ids.enabled = False
    real_empty = torch.empty
    ...
    buffers = subject.get_ref_buffers()
    assert len(buffers) == 249
    assert "token_ids" not in buffers
    assert not hasattr(subject.language_model, "_buf_token_ids")
